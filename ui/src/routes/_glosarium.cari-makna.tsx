...
import { searchGlosariumMeaning } from '#/services/glosarium.service'
...
export const Route = createFileRoute('/_glosarium/cari-makna')({
...
function RouteComponent() {
  ...
  const handleGetData = useCallback((wordFilter?: string) => {
    ...
    searchGlosariumMeaning(pagination.page, wordFilter || undefined)
      .then((data) => {
  ...
  return (
    <div className="mt-2 space-y-2">
      <GlosariumSearch placeholder="Cari Makna Kata" defaultValue={search} onChange={(value) => { handleSearchWord(value) }} />
      {loadingGetGlosarium
        ? (
            <GlosariumLoading />
          )
